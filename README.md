...
**Chain gap slider** (0–5 s, default 1 s): if two consecutive sprays are separated by less than this gap, they are linked into a **spray chain**. The history shows the chain as one row (xN prefix for N sprays), the trajectory concatenates all sprays end-to-end, and the clip gate checks combined bullet count across the whole chain — one clip is saved for the full chain. Set to 0 s to disable chaining.

### Spray history

Table of recorded sprays (one row per chain if chaining is enabled, or one per spray if the gap slider is 0). Click any row to load it into the plots and the clip player. Columns: weapon, duration (ms), sample count, net-Y (total vertical pull in mouse counts). A xN prefix indicates a chain of N sprays; a ▶ prefix means a clip is saved.
